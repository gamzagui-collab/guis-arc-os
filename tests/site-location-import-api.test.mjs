...
import {ALIAS_HEADERS,LOCATION_HEADERS,LOCATION_IMPORT_LIMITS,TEMPLATE_SHEETS} from "../worker/modules/site-location-import/contracts.js";
...
function routeWorkbook(locations,aliases=[]){const sheets=[[TEMPLATE_SHEETS[0],[["review"]]],[TEMPLATE_SHEETS[1],[LOCATION_HEADERS,...locations]],[TEMPLATE_SHEETS[2],[ALIAS_HEADERS,...aliases]],[TEMPLATE_SHEETS[3],[["review"]]],[TEMPLATE_SHEETS[4],[["review"]]],[TEMPLATE_SHEETS[5],[["review"]]]],workbookXml=`<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets>${sheets.map(([name],index)=>`<sheet name="${name}" sheetId="${index+1}" r:id="rId${index+1}"/>`).join("")}</sheets></workbook>`,rels=`<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">${sheets.map((_,index)=>`<Relationship Id="rId${index+1}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet${index+1}.xml"/>`).join("")}</Relationships>`;return zipSync({"xl/workbook.xml":strToU8(workbookXml),"xl/_rels/workbook.xml.rels":strToU8(rels),...Object.fromEntries(sheets.map(([,rows],index)=>[`xl/worksheets/sheet${index+1}.xml`,strToU8(xlsxSheet(rows))]))})}
...
function apiWorkbook({locations=[],aliases=[],locationHeaders=LOCATION_HEADERS,extraEntries={}}={}){const rows=[[["guide"]],[locationHeaders,...locations],[ALIAS_HEADERS,...aliases],[["review"]],[["drawings"]],[["rules"]]],workbookXml=`<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets>${TEMPLATE_SHEETS.map((name,index)=>`<sheet name="${name}" sheetId="${index+1}" r:id="rId${index+1}"/>`).join("")}</sheets></workbook>`,rels=`<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">${TEMPLATE_SHEETS.map((_,index)=>`<Relationship Id="rId${index+1}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet${index+1}.xml"/>`).join("")}</Relationships>`;return zipSync({"xl/workbook.xml":strToU8(workbookXml),"xl/_rels/workbook.xml.rels":strToU8(rels),...Object.fromEntries(rows.map((value,index)=>[`xl/worksheets/sheet${index+1}.xml`,strToU8(apiWorksheet(value))])),...extraEntries})}
...
test("production validation rejects cross-site stable IDs and alias targets without exposing foreign records",async()=>{
  const cases=[
    ["foreign-location-id",apiWorkbook({locations:[["foreign-location","","BUILDING","local/root","Local root",0]]}),"location_id"],
    ["foreign-alias-id",apiWorkbook({locations:[["local-root","","BUILDING","local/root","Local root",0]],aliases:[["foreign-alias","local-root","Local alias","FIELD_NAME"]]}),"alias_id"],
    ["foreign-alias-target",apiWorkbook({aliases:[["local-alias","foreign-location","Foreign target","FIELD_NAME"]]}),"location_id"]
  ];
  for(const [id,bytes,field] of cases){
    const value=await realEnv("MANAGE"),hash=crypto.createHash("sha256").update(bytes).digest("hex"),key=`sites/site-a/location-imports/${id}/original.xlsx`;
    ...
    value.db.prepare("INSERT INTO site_location_imports(id,site_id,file_name,file_hash,r2_object_key,template_version,status,created_by) VALUES(?,'site-a','locations.xlsx',?,?,'v1','UPLOADED','user-a')").run(id,hash,key);
    value.env.FILES.get=async objectKey=>objectKey===key?{size:bytes.length,arrayBuffer:async()=>bytes}:null;
    ...
    assert.equal(response.status,200,id);assert.equal(body.status,"INVALID",id);assert.equal(body.applyAllowed,false,id);
    assert.ok(body.errors.some(error=>error.code==="LOCATION_IMPORT_CROSS_SITE_REFERENCE"&&error.field===field),id);
    assert.doesNotMatch(serialized,/site-b|Secret foreign|foreign\/secret/i,id);
    ...
    assert.equal(apply.status,409,id);assert.equal(value.db.prepare("SELECT COUNT(*) count FROM site_locations WHERE site_id='site-a'").get().count,0,id);value.db.close();
  }
});

test("forged workbook site metadata cannot select the import target",async()=>{
  const value=await realEnv("MANAGE"),bytes=apiWorkbook({locationHeaders:[...LOCATION_HEADERS,"site_id"],locations:[["local-root","","BUILDING","local/root","Local root",0,"site-b"]]}),hash=crypto.createHash("sha256").update(bytes).digest("hex"),key="sites/site-a/location-imports/forged-site/original.xlsx";
