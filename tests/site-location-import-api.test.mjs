...
test("apply uses exact server recomputed location operations and commits audit, status and idempotency in one batch",async()=>{
  ...
  assert.match(sql,/INSERT INTO site_locations/);assert.match(sql,/UPDATE site_locations/);assert.doesNotMatch(sql,/site_location_aliases/);assert.match(sql,/SITE_LOCATION_IMPORT_APPLIED/);assert.match(sql,/site_location_imports/);assert.match(sql,/idempotency/i);assert.doesNotMatch(sql,/\bDELETE\b/i);
});
...
function routeWorkbook(locations){const sheets=[[TEMPLATE_SHEETS[0],[["guide"]]],[TEMPLATE_SHEETS[1],[LOCATION_HEADERS,...locations]],[TEMPLATE_SHEETS[2],[["review"]]]],workbookXml=`<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets>${sheets.map(([name],index)=>`<sheet name="${name}" sheetId="${index+1}" r:id="rId${index+1}"/>`).join("")}</sheets></workbook>`,rels=`<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">${sheets.map((_,index)=>`<Relationship Id="rId${index+1}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet${index+1}.xml"/>`).join("")}</Relationships>`;return zipSync({"xl/workbook.xml":strToU8(workbookXml),"xl/_rels/workbook.xml.rels":strToU8(rels),...Object.fromEntries(sheets.map(([,rows],index)=>[`xl/worksheets/sheet${index+1}.xml`,strToU8(xlsxSheet(rows))]))})}
...
test("status CAS guard is load-bearing and 1700 locations remain below D1 invocation limits",()=>{
  const operations=Array.from({length:1700},(_,index)=>({type:"ADD",id:`x-${index}`,after:location(`x-${index}`)})),value=applyHarness({diff:applyDiff(operations)}),response={newMasterFingerprint:"post"};
...
test("configured maximum of location-only rows remains below D1 invocation limits",()=>{
  const wide="x".repeat(500),operations=Array.from({length:5000},(_,index)=>({type:"ADD",id:`wide-location-${index}`,after:location(`wide-location-${index}`,{canonicalKey:`${wide}-${index}`,displayName:`${wide}-${index}`})})),value=applyHarness({diff:applyDiff(operations)}),response={newMasterFingerprint:"post"};
  const statements=buildLocationImportApplyStatements(value.env,{importRow:{id:"import-a",base_master_revision:0},siteId:"site-a",userId:"user-a",idempotencyKey:"key",payloadHash:"payload",diff:value.diff,requestId:"request",response,applyClaimToken:"apply-owner"});assert.ok(statements.length<=D1_LOCATION_IMPORT_LIMITS.maxStatements);assert.ok(statements.every(item=>item.args.length<=100));
});

test("production route fingerprint equals the committed canonical DB snapshot",async()=>{
  const value=await realEnv("MANAGE"),bytes=routeWorkbook([["202동","14층","1401호","거실"]]),fileHash=crypto.createHash("sha256").update(bytes).digest("hex"),key="sites/site-a/location-imports/route-import/original.xlsx";
...
const locationSheetEntry=()=>"xl/worksheets/sheet2.xml";
...
test("v2 human columns cannot address foreign stable IDs or alias targets",async()=>{
  for(const [id,label] of [["foreign-location-label","foreign-location"],["foreign-alias-label","foreign-alias"]]){
    const bytes=apiWorkbook({locations:[[label,"","",""]]}),value=await realEnv("MANAGE"),hash=crypto.createHash("sha256").update(bytes).digest("hex"),key=`sites/site-a/location-imports/${id}/original.xlsx`;
    value.db.exec("INSERT INTO sites(id,company_id,name,status) VALUES('site-b','company-a','Foreign Site','ACTIVE'); INSERT INTO site_locations(id,site_id,location_type,code,name,display_name,canonical_key,source) VALUES('foreign-location','site-b','BUILDING','FOREIGN','Secret foreign location','Secret foreign location','foreign/secret','IMPORT'); INSERT INTO site_location_aliases(id,site_id,location_id,alias_text,normalized_alias,alias_type,source) VALUES('foreign-alias','site-b','foreign-location','Secret foreign alias','secret foreign alias','FIELD_NAME','IMPORT');");
    value.db.prepare("INSERT INTO site_location_imports(id,site_id,file_name,file_hash,r2_object_key,template_version,status,created_by) VALUES(?,'site-a','locations.xlsx',?,?,'v2','UPLOADED','user-a')").run(id,hash,key);value.env.FILES.get=async objectKey=>objectKey===key?{size:bytes.length,arrayBuffer:async()=>bytes}:null;
    const response=await worker.fetch(request(`/api/v1/admin/site-locations/upload-sessions/${id}/validate`,{method:"POST",headers:value.headers}),value.env),body=await response.json(),serialized=JSON.stringify(body);
    assert.equal(response.status,200,id);assert.equal(body.status,"READY",id);assert.equal(body.applyAllowed,true,id);assert.doesNotMatch(serialized,/site-b|Secret foreign|foreign\/secret/i,id);
    const apply=await worker.fetch(request(`/api/v1/admin/site-locations/imports/${id}/apply`,{method:"POST",body:{previewHash:body.previewHash,baseMasterFingerprint:body.baseMasterFingerprint},headers:{...value.headers,"idempotency-key":`${id}-apply`}}),value.env);
    assert.equal(apply.status,200,id);assert.equal(value.db.prepare("SELECT COUNT(*) count FROM site_locations WHERE site_id='site-b'").get().count,1,id);assert.equal(value.db.prepare("SELECT COUNT(*) count FROM site_location_aliases WHERE site_id='site-a'").get().count,0,id);value.db.close();
  }
});

test("forged workbook site metadata cannot select the import target",async()=>{
  const value=await realEnv("MANAGE"),bytes=apiWorkbook({locationHeaders:[...LOCATION_HEADERS,"site_id"],locations:[["202동","14층","1401호","거실","site-b"]]}),hash=crypto.createHash("sha256").update(bytes).digest("hex"),key="sites/site-a/location-imports/forged-site/original.xlsx";
  value.db.prepare("INSERT INTO site_location_imports(id,site_id,file_name,file_hash,r2_object_key,template_version,status,created_by) VALUES('forged-site','site-a','locations.xlsx',?,?,'v1','UPLOADED','user-a')").run(hash,key);value.env.DB=transactionalD1(value.db);value.env.FILES.get=async()=>({size:bytes.length,arrayBuffer:async()=>bytes});
  let response=await worker.fetch(request("/api/v1/admin/site-locations/upload-sessions/forged-site/validate",{method:"POST",headers:value.headers}),value.env),body=await response.json();
  ...
  assert.ok(value.db.prepare("SELECT COUNT(*) count FROM site_locations WHERE site_id='site-a'").get().count>0);assert.equal(value.db.prepare("SELECT COUNT(*) count FROM site_locations WHERE site_id='site-b'").get().count,0);value.db.close();
});
...
test("successful Apply preserves a per-import hash artifact and tracks reusable signed upload keys until expiry cleanup",async()=>{
  const value=await realEnv("MANAGE"),bytes=routeWorkbook([["202동","14층","1401호","거실"]]),fileHash=crypto.createHash("sha256").update(bytes).digest("hex"),key="sites/site-a/location-imports/artifact-import/original.xlsx";
