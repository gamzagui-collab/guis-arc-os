...
function apiWorkbook({locations=[],aliases=[],locationHeaders=LOCATION_HEADERS,extraEntries={}}={}){const rows=[[["guide"]],[locationHeaders,...locations],[ALIAS_HEADERS,...aliases],[["review"]],[["drawings"]],[["rules"]]],workbookXml=`<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets>${REQUIRED_SHEETS.map((name,index)=>`<sheet name="${name}" sheetId="${index+1}" r:id="rId${index+1}"/>`).join("")}</sheets></workbook>`,rels=`<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">${REQUIRED_SHEETS.map((_,index)=>`<Relationship Id="rId${index+1}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet${index+1}.xml"/>`).join("")}</Relationships>`;return zipSync({"xl/workbook.xml":strToU8(workbookXml),"xl/_rels/workbook.xml.rels":strToU8(rels),...Object.fromEntries(rows.map((value,index)=>[`xl/worksheets/sheet${index+1}.xml`,strToU8(apiWorksheet(value))])),...extraEntries})}
