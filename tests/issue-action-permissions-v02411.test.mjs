...
test("VIEW-only assignee can read detail but receives no action controls",()=>assert.deepEqual(visibility({access:"VIEW"}),{showActionForm:false,showCompletionRequest:false,showCompletionConfirm:false,showRework:false,showSiteVerify:false}));
...
test("EDIT user with issue.act can add evidence without being the assignee",()=>{assert.equal(visibility({user:"other"}).showActionForm,true);assert.equal(visibility({user:"other",permissions:[]}).showActionForm,false)});
