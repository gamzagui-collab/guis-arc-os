...
test("service worker revision invalidates stale shell and all changed asset URLs",()=>{
  ...
  assert.match(sw,/guis-arc-integrated-v0\.27\.1-r18-shell/);
  assert.match(sw,/key\.startsWith\("guis-arc-integrated-"\)&&key!==CACHE/);
  for(const asset of ["app.js","issues.js","issues.css","app.css","manifest.webmanifest"])assert.match(sw,new RegExp(asset.replace(".","\\.")+"\\?v=0\\.27\\.1-r18"));for(const asset of ["issue-speech.js","integrated-admin.js","version.js"])assert.match(sw,new RegExp(asset.replace(".","\\.")+"\\?v=0\\.27\\.1-r18"));
  assert.match(html,/issues\.css\?v=0\.27\.1-r18/);assert.match(html,/app\.js\?v=0\.27\.1-r18/);
  assert.match(app,/version\.js\?v=0\.27\.1-r18/);assert.match(app,/issues\.js\?v=0\.27\.1-r18/);assert.match(app,/integrated-admin\.js\?v=0\.27\.1-r18/);
  assert.match(issues,/issue-speech\.js\?v=0\.27\.1-r18/);
});
