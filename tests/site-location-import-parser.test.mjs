...
const templatePath="apps/web/templates/GUI_Arc_현장위치목록_기본서식_v3.xlsx";
...
test("shipping v3 template has the exact human workbook contract and validates with no errors",()=>{
...
test("parser accepts only the human sheet, ignores review sheets, and rejects missing headers or formulas",()=>{
  ...
  assert.deepEqual(parsed.locations[0],{area:"202 동",floor:"14 층",space:"1401 호",detail:"",sourceSheetName:"01_위치목록",sourceRow:2});
...
test("build copies the shipping v3 template byte-for-byte",()=>{
  const build=spawnSync(process.execPath,["scripts/build.mjs"],{encoding:"utf8"});
  assert.equal(build.status,0,build.stderr);
  const digest=file=>crypto.createHash("sha256").update(fs.readFileSync(file)).digest("hex");
  assert.equal(digest(templatePath),digest("dist/templates/GUI_Arc_현장위치목록_기본서식_v3.xlsx"));
});
