...
test("manual UNIT selection uses the shared Draft cascade while speech auto-location stays off",()=>{const createFlow=ui.slice(ui.indexOf("async function createV3"),ui.indexOf("async function detail")),onresult=createFlow.slice(createFlow.indexOf("recognition.onresult"),createFlow.indexOf("recognition.onerror"));assert.match(ui,/filterLocationSelect\(unit,locations\.units,reconciled\.floorId\|\|reconciled\.buildingId,"UNIT"\)/);assert.match(ui,/unit\.addEventListener\("change",\(\)=>\{issueDraft=setManualUnit/);assert.match(ui,/reconcileLocationCascadePath\(resolverLocations,path\)/);assert.doesNotMatch(onresult,/form\.elements\.(?:building|floor|unit|area)\.value|addAndSelect\(|description\.value=|detailLocation\.value=/);assert.match(onresult,/speechCandidate=makeSpeechCandidate\(rawTranscript\)/)})
...
test("speech result preserves a candidate without selecting FLOOR or ROOM",()=>{const createFlow=ui.slice(ui.indexOf("async function createV3"),ui.indexOf("async function detail")),onresult=createFlow.slice(createFlow.indexOf("recognition.onresult"),createFlow.indexOf("recognition.onerror"));assert.match(createFlow,/parserSnapshot=\{building:rawParsed\.buildingLabel,floor:rawParsed\.floorLabel,unit:rawParsed\.unitLabel,room:null,detail:parsed\.detailCandidate/);assert.doesNotMatch(onresult,/addAndSelect|dispatchEvent|description\.value=|detailLocation\.value=/);assert.match(onresult,/speechCandidate=makeSpeechCandidate\(rawTranscript\)/)})

test("Issue create keeps canonical IDs or direct text mutually exclusive without dynamic master writes",()=>{const createFlow=ui.slice(ui.indexOf("async function createV3"),ui.indexOf("async function detail"));assert.match(createFlow,/location-mode-toggle/);assert.match(createFlow,/directOption\.value="__DIRECT__"/);assert.doesNotMatch(createFlow,/addAndSelect|allowDynamic:true|unitOther|areaOther/);for(const field of ["buildingLabel","floorLabel","unitLabel","roomLabel"])assert.doesNotMatch(createFlow,new RegExp(`payload\\.set\\("${field}"`));assert.match(createFlow,/buildDraftPayloadView\(issueDraft\)/);assert.match(createFlow,/payload\.set\("location",draftPayload\.location\)/);for(const id of ["buildingLocationId","floorLocationId","unitLocationId"])assert.match(createFlow,new RegExp(`payload\\.set\\("${id}",draftPayload`));assert.match(worker,/const location=qText\(form\.get\("location"\),200,"LOCATION",false\)/);assert.match(worker,/validateIssueLocationSelection\(\{building,floor,unit,room\}\);const location=/)})
