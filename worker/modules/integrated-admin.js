...
async function acceptInvitation(request,env){
 const body=await parseJson(request),invite=await findInvitation(env,body.token||body.code),pin=String(body.pin||""),birthDate=String(body.birthDate||"");if(!/^\d{4}$/.test(pin))throw new ApiError(400,"PIN_INVALID","PIN은 숫자 4자리여야 합니다.");if(!/^\d{4}-\d{2}-\d{2}$/.test(birthDate)||Number.isNaN(Date.parse(`${birthDate}T00:00:00Z`)))throw new ApiError(400,"WORKFORCE_BIRTH_DATE_REQUIRED","생년월일을 입력해 주세요.");
