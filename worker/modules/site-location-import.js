...
async function clearUploadObject(env,repository,row){if(!row?.r2_object_key||!env.FILES?.delete)return;try{await env.FILES.delete(row.r2_object_key);await repository.clearLocationImportUploadObject?.(env,{siteId:row.site_id,importId:row.id,objectKey:row.r2_object_key})}catch{}}
export async function cleanupAbandonedLocationImportObjects(env,repository,siteId){if(!env.FILES?.delete||!repository.loadLocationImportCleanupCandidates)return;const rows=await repository.loadLocationImportCleanupCandidates(env,siteId,20);for(const row of rows){const keys=[row.r2_object_key];if(row.status!=="APPLIED"&&row.file_hash)keys.push(artifactObjectKey(siteId,row.id,row.file_hash));try{await env.FILES.delete(keys.filter(Boolean));await repository.completeLocationImportUploadCleanup(env,{siteId,importId:row.id,objectKey:row.r2_object_key,status:row.status})}catch{}}}
...
export async function handleSiteLocationImportRequest(request,env,url=new URL(request.url),dependencies={}){
  ...
  if(method==="POST"&&applyMatch){
    ...
    if(row.status!=="READY")throw new ApiError(409,"LOCATION_IMPORT_STATE_INVALID","The import is not ready to apply.");assertUploadObject(row,auth.siteId);
  ...
  if(method==="POST"&&validation){
    ...
    try{
      ...
      let workbook;try{workbook=parseSiteLocationWorkbook(bytes)}catch(error){closed=true;await repository.updateLocationImportValidation(env,{id:row.id,siteId:auth.siteId,status:"INVALID",counts:{error:1}});await clearUploadObject(env,repository,row);throw new ApiError(400,error.code||"LOCATION_XLSX_INVALID",error.message)}
