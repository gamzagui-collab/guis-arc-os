...
const TEMPLATE="GUI_Arc_현장위치목록_기본서식_v3.xlsx";
...
export async function handleSiteLocationImportRequest(request,env,url=new URL(request.url),dependencies={}){
  ...
  if(method==="GET"&&path==="/api/v1/admin/site-locations/summary"){const auth=await authorizeRequest(request,env,{required:"VIEW"}),[count,history,appliedFiles]=await Promise.all([repository.countActiveFixedLocations(env,auth.siteId),repository.loadRecentLocationImports(env,auth.siteId,20),repository.loadAppliedLocationFiles?.(env,auth.siteId,2)??[]]);return json({activeFixedLocationCount:count,lastSuccessfulImport:history.find(item=>item.status==="APPLIED")??null,currentAppliedFile:appliedFiles[0]??null,previousAppliedFile:appliedFiles[1]??null})}
  ...
  const originalMatch=path.match(/^\/api\/v1\/admin\/site-locations\/imports\/([^/]+)\/original$/);
  if(method==="GET"&&originalMatch){const auth=await authorizeRequest(request,env,{required:"VIEW"}),row=await repository.getRetainedAppliedLocationFile?.(env,auth.siteId,originalMatch[1]);if(!row)throw new ApiError(404,"LOCATION_IMPORT_FILE_NOT_FOUND","보관 중인 적용 파일을 찾을 수 없습니다.");const object=await env.FILES.get(row.artifact_object_key);if(!object)throw new ApiError(404,"LOCATION_IMPORT_FILE_NOT_FOUND","보관 중인 적용 파일을 찾을 수 없습니다.");const bytes=await object.arrayBuffer(),headers=new Headers({"content-type":MIME,"content-disposition":`attachment; filename*=UTF-8''${encodeURIComponent(row.file_name)}`,"cache-control":"private, no-store"});return new Response(bytes,{status:200,headers})}
  const applyMatch=path.match(/^\/api\/v1\/admin\/site-locations\/imports\/([^/]+)\/apply$/);
  if(method==="POST"&&applyMatch){
    const auth=await authorizeRequest(request,env,{required:"MANAGE",write:true}),body=await parseJson(request),idempotencyKey=String(request.headers.get("idempotency-key")||"").trim(),renameDecisions=Array.isArray(body.renameDecisions)?body.renameDecisions:[];
    ...
    const previewHash=String(body.previewHash||""),baseMasterFingerprint=String(body.baseMasterFingerprint||""),payloadHash=await hash(new TextEncoder().encode(JSON.stringify({importId:row.id,siteId:auth.siteId,previewHash,baseMasterFingerprint,renameDecisions})).buffer);
    ...
    const current=await repository.loadLocationMasterSnapshot(env,auth.siteId),initial=validateLocationImport({siteId:auth.siteId,workbook,current}),identityGuards=await repository.loadLocationImportIdentityGuards?.(env,auth.siteId,{locations:initial.normalized.locations,aliases:[]})??{},checked=validateLocationImport({siteId:auth.siteId,workbook,current,identityGuards}),baseDiff=buildLocationImportDiff({siteId:auth.siteId,normalized:checked.normalized,current}),diff=buildLocationImportDiff({siteId:auth.siteId,normalized:checked.normalized,current,renameDecisions}),capacity=assessLocationImportApplyCapacity(diff);
    if(checked.errors.length||diff.counts.error||!capacity.allowed||previewHash!==row.preview_hash||baseDiff.previewHash!==row.preview_hash||baseMasterFingerprint!==row.base_master_fingerprint||Number(current.revision)!==Number(row.base_master_revision))throw new ApiError(409,"LOCATION_IMPORT_PREVIEW_STALE","The file, master, or preview changed.");
    const result=await applyLocationImport({env,repository,importRow:row,siteId:auth.siteId,userId:auth.userId,idempotencyKey,payloadHash,diff,fileHash,requestId:requestId(request),currentSnapshot:current,fileBytes:bytes,uploadObjectKey:row.r2_object_key,artifactObjectKey:artifactObjectKey(auth.siteId,row.id,fileHash)});await retainLatestAppliedLocationArtifacts(env,repository,auth.siteId);return json(result);
  }
  ...
  if(method==="POST"&&validation){
    const auth=await authorizeRequest(request,env,{required:"MANAGE",write:true}),row=await repository.getLocationImport(env,auth.siteId,validation[1]);if(!row)throw new ApiError(404,"LOCATION_IMPORT_NOT_FOUND","현재 현장의 업로드를 찾을 수 없습니다.");if(!["UPLOADED","INVALID","READY","VALIDATING"].includes(row.status))throw new ApiError(409,"LOCATION_IMPORT_STATE_INVALID","현재 상태에서는 다시 검증할 수 없습니다.");assertUploadObject(row,auth.siteId);const validationClaimToken=crypto.randomUUID();if(!await repository.beginLocationImportValidation(env,auth.siteId,row.id,validationClaimToken))throw new ApiError(409,"LOCATION_IMPORT_STATE_INVALID","업로드 상태가 변경되었습니다. 새로고침해 주세요.");
    let closed=false;const closeInvalid=async(errorCode)=>{const finalized=await repository.updateLocationImportValidation(env,{id:row.id,siteId:auth.siteId,status:"INVALID",counts:{error:1},validationClaimToken});closed=true;if(!finalized)throw new ApiError(409,"LOCATION_IMPORT_STATE_INVALID","검증 중 업로드 상태가 변경되었습니다.");await removeUploadObject(env,row);throw new ApiError(errorCode==="LOCATION_IMPORT_OBJECT_SIZE_INVALID"?413:errorCode==="LOCATION_IMPORT_HASH_MISMATCH"?409:400,errorCode,errorCode==="LOCATION_IMPORT_OBJECT_SIZE_INVALID"?"업로드된 파일 크기가 허용 범위를 벗어났습니다.":errorCode==="LOCATION_IMPORT_HASH_MISMATCH"?"업로드 파일 확인값이 일치하지 않습니다.":"올바른 XLSX 파일이 아닙니다.")};
    try{
      const object=await env.FILES.get(row.r2_object_key);if(!object){closed=true;await repository.updateLocationImportValidation(env,{id:row.id,siteId:auth.siteId,status:"INVALID",counts:{error:1},validationClaimToken});throw new ApiError(409,"LOCATION_IMPORT_OBJECT_MISSING","업로드 파일을 찾을 수 없습니다.")}let objectSize;try{objectSize=assertObjectSize(object)}catch{await closeInvalid("LOCATION_IMPORT_OBJECT_SIZE_INVALID")}const bytes=await object.arrayBuffer(),signature=new Uint8Array(bytes,0,Math.min(4,bytes.byteLength));if(bytes.byteLength!==objectSize||signature[0]!==0x50||signature[1]!==0x4b||signature[2]!==0x03||signature[3]!==0x04)await closeInvalid("LOCATION_XLSX_INVALID");if(await hash(bytes)!==row.file_hash)await closeInvalid("LOCATION_IMPORT_HASH_MISMATCH");
      let workbook;try{workbook=parseSiteLocationWorkbook(bytes)}catch(error){const finalized=await repository.updateLocationImportValidation(env,{id:row.id,siteId:auth.siteId,status:"INVALID",counts:{error:1},validationClaimToken});closed=true;if(!finalized)throw new ApiError(409,"LOCATION_IMPORT_STATE_INVALID","검증 중 업로드 상태가 변경되었습니다.");await removeUploadObject(env,row);throw new ApiError(400,error.code||"LOCATION_XLSX_INVALID",error.message)}
      const current=await repository.loadLocationMasterSnapshot(env,auth.siteId),initial=validateLocationImport({siteId:auth.siteId,workbook,current}),identityGuards=await repository.loadLocationImportIdentityGuards?.(env,auth.siteId,{locations:initial.normalized.locations,aliases:[]})??{},checked=validateLocationImport({siteId:auth.siteId,workbook,current,identityGuards}),diff=buildLocationImportDiff({siteId:auth.siteId,normalized:checked.normalized,current}),capacity=assessLocationImportApplyCapacity(diff),capacityErrors=capacity.allowed?[]:[{code:"LOCATION_IMPORT_APPLY_LIMIT",field:"workbook",sourceSheetName:null,sourceRow:null}],errors=[...checked.errors,...diff.operations.filter(item=>item.type==="ERROR"),...capacityErrors].slice(0,100),counts={...diff.counts,error:checked.errors.length+diff.counts.error+capacityErrors.length},status=counts.error?"INVALID":"READY";
      if(!await repository.updateLocationImportValidation(env,{id:row.id,siteId:auth.siteId,status,baseMasterFingerprint:diff.baseMasterFingerprint,baseMasterRevision:current.revision,previewHash:diff.previewHash,counts,validationClaimToken}))throw new ApiError(409,"LOCATION_IMPORT_STATE_INVALID","검증 중 업로드 상태가 변경되었습니다.");closed=true;if(status==="INVALID")await removeUploadObject(env,row);return json({importId:row.id,status,applyAllowed:status==="READY",fileHash:row.file_hash,baseMasterFingerprint:diff.baseMasterFingerprint,baseMasterRevision:current.revision,previewHash:diff.previewHash,counts,operations:diff.operations.filter(item=>item.type!=="ERROR"),errors});
    }catch(error){if(!closed)try{await repository.updateLocationImportValidation(env,{id:row.id,siteId:auth.siteId,status:"FAILED",counts:{error:1},validationClaimToken})}catch{}throw error}
  }
  return null;
}
