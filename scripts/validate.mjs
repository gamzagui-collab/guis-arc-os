...
for(const [file,token] of[["package.json",'"version": "0.27.1"'],["package-lock.json",'"version": "0.27.1"'],["wrangler.integration.toml",'APP_VERSION = "0.27.1"'],["worker/version.js",'VERSION="0.27.1"'],["apps/web/assets/version.js",'VERSION="0.27.1"'],["apps/web/assets/app.js","issues.js?v=0.27.1-r3"],["apps/web/index.html","v0.27.1"],["apps/web/service-worker.js","v0.27.1-r3-shell"]])if(!read(file).includes(token))throw new Error(`Version surface mismatch: ${file}`);
...
for(const forbidden of ["location_id","parent_location_id","canonical_key","location_type","sort_order","source","alias_id"])if(phaseAOpenXml.includes(forbidden))throw new Error(`Phase A template leaks internal field ${forbidden}`);
