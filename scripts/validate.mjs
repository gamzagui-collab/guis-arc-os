...
for(const [file,token] of[["package.json",'"version": "0.27.1"'],["package-lock.json",'"version": "0.27.1"'],["wrangler.integration.toml",'APP_VERSION = "0.27.1"'],["worker/version.js",'VERSION="0.27.1"'],["apps/web/assets/version.js",'VERSION="0.27.1"'],["apps/web/assets/app.js","issues.js?v=0.27.1-r1"],["apps/web/index.html","v0.27.1"],["apps/web/service-worker.js","v0.27.1-r1-shell"]])if(!read(file).includes(token))throw new Error(`Version surface mismatch: ${file}`);
...
for(const token of ["canonical_key TEXT","source TEXT NOT NULL DEFAULT 'LEGACY'","CREATE TABLE site_location_aliases","UNIQUE(site_id,normalized_alias)","CREATE TABLE site_location_imports","base_master_revision","post_master_fingerprint","CREATE TABLE site_location_import_idempotency","FOREIGN KEY(site_id,location_id) REFERENCES site_locations(site_id,id)"])if(!read(phaseAMigration).includes(token))throw new Error(`Phase A schema contract missing ${token}`);
