...
for(const [file,token] of[["package.json",'"version": "0.27.1"'],["package-lock.json",'"version": "0.27.1"'],["wrangler.integration.toml",'APP_VERSION = "0.27.1"'],["worker/version.js",'VERSION="0.27.1"'],["apps/web/assets/version.js",'VERSION="0.27.1"'],["apps/web/assets/app.js","issues.js?v=0.27.1-r33"],["apps/web/index.html","v0.27.1"],["apps/web/service-worker.js","v0.27.1-r33-shell"]])if(!read(file).includes(token))throw new Error(`Version surface mismatch: ${file}`);
...
const phaseAMigration="database/migrations/0036_site_location_master_import.sql",phaseATemplate="apps/web/templates/GUI_Arc_현장위치목록_기본서식_v3.xlsx";
...
if(JSON.stringify(phaseAWorkbook.workbookMeta.sheetNames)!==JSON.stringify(TEMPLATE_SHEETS)||TEMPLATE_SHEETS.length!==2)throw new Error("Phase A template exact sheet contract mismatch");
...
if(JSON.stringify(SIMPLE_LOCATION_HEADERS)!==JSON.stringify(["건물/구역","층","호/공간"]))throw new Error("Phase A exact header contract mismatch");
