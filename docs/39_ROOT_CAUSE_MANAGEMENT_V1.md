...
- 확정된 분석의 원인, 원인 note 또는 재발 방지 메모가 실질적으로 변경되면 분석은 `DRAFT`로 돌아가며 확정자와 확정 시각을 초기화한다.
- 원인 배열 순서만 다른 동일한 의미의 저장은 변경으로 보지 않는다.
- 미확정 분석은 `canDraft`, 확정 분석은 `canConfirm`을 기준으로 최종 `canEdit`을 계산하며 UI와 저장 API가 이를 공유한다.
- DRAFT로 돌아간 분석은 재확정 전 집계에서 제외된다.
